...
export default function IngestPage() {
  ...
  const fetchSeoulArchive = async () => {
    if (!window.confirm('서울역사아카이브 근현대서울사진을 수집합니다.\n사회생활·문화예술·여가/체육·건설·교통 등 39개 카테고리\n수 시간 소요될 수 있습니다.\n시작하시겠습니까?')) return
    try {
      const res = await fetch('/api/ingest/fetch-seoul-archive', {
        method: 'POST',
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <button
            onClick={fetchSeoulArchive}
            style={{
              background: 'transparent', border: '1px solid #7a9a60', color: '#7a9a60',
              borderRadius: '7px', padding: '0.6rem 1.4rem', cursor: 'pointer',
              fontSize: '0.85rem', fontWeight: '600', letterSpacing: '0.04em',
            }}
          >
          ...
          <span style={{ color: '#8a7080', fontSize: '0.75rem' }}>
            근현대서울사진 39개 카테고리 — 사회생활·문화예술·여가·극장·체육 등 · 수 시간 소요
          </span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <button
